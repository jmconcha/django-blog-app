...
def front_page(request):
    blogs = Blog.objects.filter(
        status=Blog.ACTIVE).order_by('-created_at')[:10]
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.add_message(
                request, messages.WARNING, "Your username and password didn't match. Please try again.")
    else:
    ...
    if request.method == 'POST':
        ...
        if not errors:
            ...
            UserProfile.objects.create(
                user=user, bio='', location='', birth_date=None)
            login(request, user)
