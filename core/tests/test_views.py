...
class FrontPageViewTests(TestCase):
    ...
    def test_front_page_with_draft_blogs(self):
        ...
        for count in range(3):
            create_blog(
                user, title=f'Test Title {count}', body=f'Test Body {count}')
    ...
    def test_front_page_with_active_blogs(self):
        ...
        for count in range(3):
            blog = create_blog(
                user, title=f'Blog Title {count}', body=f'This is blog body {count}', status=Blog.ACTIVE)
            blogs.append(blog)
...
class LoginUserViewTests(TestCase):
    ...
    def test_login_with_unregistered_user(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Your username and password didn't match. Please try again.", html=True)
